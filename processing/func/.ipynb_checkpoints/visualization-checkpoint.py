...
def show_wsi_with_mark(uuid,show = True,get_pic = False,cmap = 'gray'):
    ...
    #         mask_coords,classes = ep.AnnotationParser(xml_path)
        ...
        print(f'{uuid} error occured:{e}')
    
def show_wsis_with_marks(uuid_list:list,figsize,ret = False):
    '''
    args:
        ret:
            if True: returns figseq:4darray & title:list
            if False: directly show in output cell
    '''
    t = time.time()
    ...
    for uuid in tqdm(uuid_list):
        ...
        try:
            ...
            t = [str(uuid) + f'--{not_well_marked[uuid]}','T','nonT']
            title.extend(t)
            figseq.extend([thumbnail,T_marked_thumb,non_T_marked_thumb])
        except Exception as e:
            print(f'{uuid} has an error in batch processing --->{e}')
    if ret:
        return figseq,title
    else:
        num_of_pics = len(figseq)//3
