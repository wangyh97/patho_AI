...
class Dataset():
    def __init__(self, npy_file, transform=None):
        data_df = np.load(npy_file,allow_pickle = True).item()['full_list']
        self.files_list = sum(data_df['img_list'],[]) 
...
class DataSetWrapper(object):
    ...
    def __init__(self, scale, batch_size, num_workers, valid_size, input_shape, s):
    ...
    def get_data_loaders(self):
        ...
        train_dataset = Dataset(npy_file=f'../../config/data_segmentation_csv/{self.scale}X_full.npy', transform=SimCLRDataTransform(data_augment))
        print(f'get {self.scale}X grouping info successfully')
