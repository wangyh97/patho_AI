    ...
    for epoch in range(args.start_epoch+1,num_epochs):
        ...
        for phase in ['train','val']:
            ...
            for i, (inputs, labels) in enumerate(dataloders[phase]):
                ...
                if use_gpu:
                    inputs = Variable(inputs.to(cudas[args.gpus]))
                    labels = Variable(labels.to(cudas[args.gpus]))
                else:
...
if __name__ == '__main__':
    ...
    cudas = {
        ...
        "2" : torch.device('cuda:2'),
        "3" : torch.device('cuda:3')
    }
    ...
    if use_gpu:
        ...
        model = model.to(cudas[args.gpus])
